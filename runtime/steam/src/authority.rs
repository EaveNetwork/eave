...
use crate::{
	EaveTreasuryPalletId, AccountId, AccountIdConversion, AuthoritysOriginId, BadOrigin, BlockNumber, DispatchResult,
...
impl orml_authority::AsOriginId<Origin, OriginCaller> for AuthoritysOriginId {
	fn into_origin(self) -> OriginCaller {
		match self {
			...
			AuthoritysOriginId::EaveTreasury => Origin::signed(EaveTreasuryPalletId::get().into_account())
				.caller()
				.clone(),
			...
			AuthoritysOriginId::HomaTreasury => Origin::signed(HomaTreasuryPalletId::get().into_account())
				.caller()
				.clone(),
			AuthoritysOriginId::DSWF => Origin::signed(TreasuryPalletId::get().into_account())
			.caller()
			.clone(),
		}
	}
