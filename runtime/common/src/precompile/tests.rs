...
use crate::precompile::{
	mock::{
		eave_evm_address, alice, alice_evm_addr, eusd_evm_address, bob, bob_evm_addr, erc20_address_not_exists,
		get_task_id, lp_eave_eusd_evm_address, new_test_ext, renbtc_evm_address, run_to_block, Balances, DexModule,
		DexPrecompile, Event as TestEvent, MultiCurrencyPrecompile, Oracle, OraclePrecompile, Origin, Price,
		ScheduleCallPrecompile, System, Test, ALICE, EUSD, INITIAL_BALANCE, RENBTC,
	},
...
use acala_primitives::{Balance, PREDEPLOY_ADDRESS_START};
...
#[test]
fn precompile_filter_works_on_eave_precompiles() {
...
fn multicurrency_precompile_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Token
		context.caller = eave_evm_address();
		...
		expected_output[27..32].copy_from_slice(&b"Eave"[..]);
		assert_eq!(output, expected_output);
		assert_eq!(used_gas, 0);

		// DexShare
		context.caller = lp_eave_eusd_evm_address();
		let (reason, output, used_gas) = MultiCurrencyPrecompile::execute(&input, None, &context).unwrap();
		assert_eq!(reason, ExitSucceed::Returned);
		let mut expected_output = [0u8; 32];
		expected_output[9..32].copy_from_slice(&b"LP Eave - Eave Dollar"[..]);
		assert_eq!(output, expected_output);
		assert_eq!(used_gas, 0);
		...
		assert_eq!(reason, ExitSucceed::Returned);
		let mut expected_output = [0u8; 32];
		expected_output[29..32].copy_from_slice(&b"EAVE"[..]);
		assert_eq!(output, expected_output);
		assert_eq!(used_gas, 0);
		...
		assert_eq!(reason, ExitSucceed::Returned);
		let mut expected_output = [0u8; 32];
		expected_output[21..32].copy_from_slice(&b"LP_EAVE_EUSD"[..]);
		assert_eq!(output, expected_output);
		assert_eq!(used_gas, 0);
...
fn schedule_call_precompile_should_work() {
	new_test_ext().execute_with(|| {
		...
		U256::from(alice_evm_addr().as_bytes()).to_big_endian(&mut input[4 + 1 * 32..4 + 2 * 32]);
		// target
		U256::from(eave_evm_address().as_bytes()).to_big_endian(&mut input[4 + 2 * 32..4 + 3 * 32]);
		// value
...
fn dex_precompile_get_liquidity_should_work() {
	new_test_ext().execute_with(|| {
		// enable RENBTC/EUSD
		assert_ok!(DexModule::enable_trading_pair(Origin::signed(ALICE), RENBTC, EUSD,));
		...
		U256::from_big_endian(&renbtc_evm_address().as_bytes()).to_big_endian(&mut input[4 + 1 * 32..4 + 2 * 32]);
		// EUSD
		U256::from_big_endian(&eusd_evm_address().as_bytes()).to_big_endian(&mut input[4 + 2 * 32..4 + 3 * 32]);
...
fn dex_precompile_get_swap_target_amount_should_work() {
	new_test_ext().execute_with(|| {
		...
		// EUSD
		U256::from_big_endian(&eusd_evm_address().as_bytes()).to_big_endian(&mut input[4 + 5 * 32..4 + 6 * 32]);
...
fn dex_precompile_swap_with_exact_supply_should_work() {
	new_test_ext().execute_with(|| {
		...
		// EUSD
		U256::from_big_endian(&eusd_evm_address().as_bytes()).to_big_endian(&mut input[4 + 7 * 32..4 + 8 * 32]);
