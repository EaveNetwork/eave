...
use crate::precompile::mock::{
	...
	Price, ScheduleCallPrecompile, Test, EAVE_ERC20_ADDRESS, ALICE, EUSD, XBTC,
};
...
fn schedule_call_precompile_should_work() {
	new_test_ext().execute_with(|| {
		...
		// target
		U256::from(EAVE_ERC20_ADDRESS).to_big_endian(&mut input[3 * 32..4 * 32]);
		// value
		...
		let from_account = <Test as eave_pallet_evm::Config>::AddressMapping::get_account_id(&alice());
		let to_account = <Test as eave_pallet_evm::Config>::AddressMapping::get_account_id(&bob());
...
fn dex_precompile_get_liquidity_should_work() {
	new_test_ext().execute_with(|| {
		// enable XBTC/EUSD
		assert_ok!(Dexpallet::enable_trading_pair(Origin::signed(ALICE), XBTC, EUSD,));
