...
/// Maybe Steam Dev full select chain.
...
pub const STEAM_RUNTIME_NOT_AVAILABLE: &str =
	"Steam runtime is not available. Please compile the node with `--features with-steam-runtime` to enable it.";
pub const EAVE_RUNTIME_NOT_AVAILABLE: &str =
	"Eave runtime is not available. Please compile the node with `--features with-eave-runtime` to enable it.";
...
> {
	...
	if config.chain_spec.is_steam_dev() || config.chain_spec.is_steam() {
		#[cfg(feature = "with-steam-runtime")]
		{
		...
		#[cfg(feature = "with-eave-runtime")]
		{
			let PartialComponents {
				...
				task_manager,
				..
			} = new_partial::<eave_runtime::RuntimeApi, EaveExecutor>(config, false, false)?;
			Ok((Arc::new(Client::Eave(client)), backend, import_queue, task_manager))
		}
		#[cfg(not(feature = "with-eave-runtime"))]
		Err(EAVE_RUNTIME_NOT_AVAILABLE.into())
	}
}

#[cfg(feature = "with-steam-runtime")]
fn inner_steam_dev(config: Configuration, instant_sealing: bool) -> Result<TaskManager, ServiceError> {
	...
	let select_chain =
		maybe_select_chain.expect("In steam dev mode, `new_partial` will return some `select_chain`; qed");
...
#[cfg(feature = "with-steam-runtime")]
pub fn steam_dev(config: Configuration, instant_sealing: bool) -> Result<TaskManager, ServiceError> {
	inner_steam_dev(config, instant_sealing)
}