...
//! Eave dev service. Specialized wrapper over substrate service.
...
/// Eave's full Grandpa block import.
...
pub fn new_partial<RuntimeApi, Executor>(
	...
	PartialComponents<
		...
		(
			impl Fn(eave_rpc::DenyUnsafe, eave_rpc::SubscriptionTaskExecutor) -> eave_rpc::RpcExtension,
			(
...
{
	if !test {
		// If we're using prometheus, use a registry with a prefix of `eave`.
	...
	let (rpc_extensions_builder, rpc_setup) = {
		...
		let rpc_extensions_builder = move |deny_unsafe, subscription_executor| -> eave_rpc::RpcExtension {
			let deps = eave_rpc::FullDeps {
				...
				deny_unsafe,
				babe: eave_rpc::BabeDeps {
					babe_config: babe_config.clone(),
					shared_epoch_changes: shared_epoch_changes.clone(),
					keystore: keystore.clone(),
				},
				grandpa: eave_rpc::GrandpaDeps {
	...
	Executor: NativeExecutionDispatch + 'static,
{
	...
	let light_deps = eave_rpc::LightDeps {
...
pub fn build_light(config: Configuration) -> Result<TaskManager, ServiceError> {
	if config.chain_spec.is_eave() {
		#[cfg(feature = "with-eave-runtime")]
		return new_light::<eave_runtime::RuntimeApi, EaveExecutor>(config).map(|r| r.0);
		#[cfg(not(feature = "with-eave-runtime"))]
		Err("Eave runtime is not available. Please compile the node with `--features with-eave-runtime` to enable it.".into())
	} else {
		#[cfg(feature = "with-steam-runtime")]
	...
	if config.chain_spec.is_eave() {
		#[cfg(feature = "with-eave-runtime")]
		{
			let (task_manager, _, client, _, _, network_status_sinks) =
				new_full::<eave_runtime::RuntimeApi, EaveExecutor>(config, instant_sealing, test)?;
			Ok((Arc::new(Client::Eave(client)), network_status_sinks, task_manager))
		}
		#[cfg(not(feature = "with-eave-runtime"))]
		Err("Eave runtime is not available. Please compile the node with `--features with-eave-runtime` to enable it.".into())
	} else {
