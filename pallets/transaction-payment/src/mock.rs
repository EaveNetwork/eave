...
parameter_types! {
	...
	pub EnabledTradingPairs : Vec<TradingPair> = vec![TradingPair::new(EUSD, EAVE), TradingPair::new(EUSD, DOT)];
}
...
parameter_types! {
	pub AllNonNativeCurrencyIds: Vec<CurrencyId> = vec![EUSD, DOT];
	pub MaxSlippageSwapWithDEX: Ratio = Ratio::one();
	pub const StableCurrencyId: CurrencyId = EUSD;
...
impl Default for ExtBuilder {
	fn default() -> Self {
		Self {
			endowed_accounts: vec![(ALICE, EUSD, 10000), (ALICE, DOT, 1000)],
			base_weight: 0,
			byte_fee: 2,
			weight_to_fee: 1,
		}
