...
use mock::{
	AccountId, BlockWeights, Call, Currencies, DEXModule, ExtBuilder, Origin, Runtime, TransactionPayment, EAVE, ALICE,
	EUSD, BOB, DOT,
};
...
const CALL: &<Runtime as frame_system::Config>::Call =
	&Call::Currencies(module_currencies::Call::transfer(BOB, EUSD, 12));
...
fn charges_fee() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(EAVE, &ALICE), (100000 - fee).into());
		...
		assert_eq!(
			Currencies::free_balance(EAVE, &ALICE),
...
fn charges_fee_when_pre_dispatch_and_native_currency_is_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(ChargeTransactionPayment::<Runtime>::from(0)
			...
			.is_ok());
		assert_eq!(Currencies::free_balance(EAVE, &ALICE), 100000 - fee);
	});
}
...
fn refund_fee_according_to_actual_when_post_dispatch_and_native_currency_is_enough() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(EAVE, &ALICE), 100000 - fee + refund);
	});
}
...
fn charges_fee_when_validate_and_native_is_not_enough() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(<Currencies as MultiCurrency<_>>::transfer(EUSD, &ALICE, &BOB, 1000));
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(EAVE, &BOB), 0);
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(EUSD, &BOB), 1000);
		...
		assert_eq!(DEXModule::get_liquidity_pool(EAVE, EUSD), (10000, 1000));
		...
		assert_eq!(Currencies::free_balance(EAVE, &BOB), 0);
		assert_eq!(Currencies::free_balance(EUSD, &BOB), 749);
		assert_eq!(DEXModule::get_liquidity_pool(EAVE, EUSD), (10000 - 2000, 1251));
	});
}
...
fn set_default_fee_token_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(TransactionPayment::default_fee_currency_id(&ALICE), Some(EUSD));
...
fn charge_fee_by_default_fee_token() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DEXModule::get_liquidity_pool(EAVE, EUSD), (10000, 1000));
		assert_eq!(DEXModule::get_liquidity_pool(DOT, EUSD), (100, 1000));
		assert_ok!(TransactionPayment::set_default_fee_token(
			Origin::signed(BOB),
			Some(DOT)
		));
		...
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(EAVE, &BOB), 0);
		assert_eq!(<Currencies as MultiCurrency<_>>::free_balance(EUSD, &BOB), 0);
		...
		assert_eq!(Currencies::free_balance(EAVE, &BOB), 0);
		assert_eq!(Currencies::free_balance(EUSD, &BOB), 0);
		assert_eq!(Currencies::free_balance(DOT, &BOB), 100 - 34);
		assert_eq!(DEXModule::get_liquidity_pool(EAVE, EUSD), (10000 - 2000, 1251));
		assert_eq!(DEXModule::get_liquidity_pool(DOT, EUSD), (100 + 34, 1000 - 251));
	});
}
