...
use eave_primitives::{Amount, Balance, CurrencyId};
use sp_runtime::{
...
pub enum PoolId {
	/// Rewards(EAVE) pool for users who open SHY
	Loans(CurrencyId),
	/// Rewards(EAVE) pool for market makers who provide dex liquidity
	DexIncentive(CurrencyId),
	/// Rewards(EUSD) pool for liquidators who provide dex liquidity to
	/// participate automatic liquidation
	DexSaving(CurrencyId),
	/// Rewards(EAVE) pool for users who staking by Slip protocol
	Slip,
}
...
pub mod module {
		...
		frame_system::Config + orml_rewards::Config<Share = Balance, Balance = Balance, PoolId = PoolId>
	{
		...
		/// The incentive reward type (should be EAVE)
		#[pallet::constant]
		type IncentiveCurrencyId: Get<CurrencyId>;

		/// The saving reward type (should be EUSD)
		#[pallet::constant]
		...
		/// SHY treasury to issue rewards in EUSD
