...
use mock::{Airdrop, Event, ExtBuilder, Origin, System, EAVE, ALICE, BOB, CHARLIE, BEAM};
...
fn update_airdrop_work() {
	ExtBuilder::default().build().execute_with(|| {
		System::set_block_number(1);
		assert_ok!(Airdrop::airdrop(Origin::root(), ALICE, EAVE, 10000));
		assert_ok!(Airdrop::airdrop(Origin::root(), ALICE, EAVE, 10000));
		assert_eq!(Airdrop::airdrops(ALICE, EAVE), 20000);
		assert_noop!(Airdrop::update_airdrop(Origin::signed(BOB), ALICE, EAVE, 0), BadOrigin,);
		assert_ok!(Airdrop::update_airdrop(Origin::root(), ALICE, EAVE, 0));
		let update_airdrop_event = Event::airdrop(RawEvent::UpdateAirdrop(ALICE, EAVE, 0));
		assert!(System::events()
			.iter()
			.any(|record| record.event == update_airdrop_event));
		assert_eq!(Airdrop::airdrops(ALICE, EAVE), 0);
	});
}
...
fn genesis_config_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Airdrop::airdrops(CHARLIE, EAVE), 80);
	});
}
