...
impl PriceProvider<CurrencyId> for MockPriceSource {
	fn get_relative_price(base: CurrencyId, quote: CurrencyId) -> Option<Price> {
		match (base, quote) {
			(EUSD, BTC) => RELATIVE_PRICE.with(|v| *v.borrow_mut()),
			(BTC, EUSD) => RELATIVE_PRICE.with(|v| *v.borrow_mut()),
			_ => None,
		}
	}
...
parameter_types! {
	...
	pub EnabledTradingPairs : Vec<TradingPair> = vec![TradingPair::new(EUSD, BTC), TradingPair::new(EUSD, DOT)];
}
