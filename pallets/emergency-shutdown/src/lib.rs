...
use eave_primitives::{Balance, CurrencyId};
use sp_runtime::{traits::Zero, FixedPointNumber};
use sp_std::prelude::*;
use eave_pallet_support::{AuctionManager, SHYTreasury, EmergencyShutdown, PriceProvider, Ratio};
...
pub mod module {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + eave_pallet_loans::Config {
		...
		pub fn open_collateral_refund(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
			...
			for currency_id in collateral_currency_ids {
				...
				// there's on debit in SHY
					...
					<eave_pallet_loans::Module<T>>::total_positions(currency_id).debit.is_zero(),
