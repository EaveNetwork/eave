...
fn get_price_from_oracle() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(PricesModule::get_price(EAVE), Some(Price::zero()));
	});
}
...
fn get_price_of_lp_token_currency_id() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(MockDEX::get_liquidity_pool(EUSD, DOT), (10000, 200));
		assert_eq!(
			PricesModule::get_price(LP_EUSD_DOT),
			None
		);
		assert_ok!(Tokens::deposit(LP_EUSD_DOT, &1, 100));
		assert_eq!(Tokens::total_issuance(LP_EUSD_DOT), 100);
		assert_eq!(PricesModule::get_price(EUSD), Some(Price::saturating_from_rational(1000000u128, 1)));
		assert_eq!(
			PricesModule::get_price(LP_EUSD_DOT),
			Some(Price::saturating_from_rational(200000000u128, 1))	// 10000/100 * Price::saturating_from_rational(1000000u128, 1) * 2
		);

		assert_eq!(MockDEX::get_liquidity_pool(BTC, EUSD), (0, 0));
		assert_eq!(
			PricesModule::get_price(LP_BTC_EUSD),
			None
		);
	});
}
...
fn get_relative_price_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			PricesModule::get_relative_price(DOT, EUSD),
			Some(Price::saturating_from_rational(10000, 1)) /* 1DOT = 100EUSD, right shift the decimal point (12-10)
			                                                 * places */
		);
		assert_eq!(
			PricesModule::get_relative_price(BTC, EUSD),
			Some(Price::saturating_from_rational(500000000, 1)) /* 1BTC = 50000EUSD, right shift the decimal point
			                                                     * (12-8) places */
		);
		assert_eq!(
		...
		assert_eq!(
			PricesModule::get_relative_price(EUSD, EUSD),
			Some(Price::saturating_from_rational(1, 1)) // 1EUSD = 1EUSD, right shift the decimal point (10-10) places
		);
		assert_eq!(PricesModule::get_relative_price(EUSD, EAVE), None);
	});
}
