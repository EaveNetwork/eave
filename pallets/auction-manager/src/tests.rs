...
fn collateral_auction_bid_handler_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 1000);
		...
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 995);
		...
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 1000);
		assert_eq!(Tokens::free_balance(EUSD, &CAROL), 990);
		...
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 900);
		assert_eq!(Tokens::free_balance(EUSD, &CAROL), 1000);
...
fn debit_auction_bid_handler_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(SHYTreasuryModule::surplus_pool(), 100);
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 1000);
		assert_eq!(Tokens::free_balance(EUSD, &CAROL), 900);
		let bob_ref_count_2 = System::consumers(&BOB);
		assert_eq!(bob_ref_count_2, bob_ref_count_1 - 1);
		let carol_ref_count_1 = System::consumers(&CAROL);
		assert_eq!(carol_ref_count_1, carol_ref_count_0 + 1);
...
fn surplus_auction_bid_handler_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AuctionManagerModule::new_surplus_auction(100));
		assert_eq!(Tokens::free_balance(EAVE, &BOB), 1000);
		...
		assert_eq!(Tokens::free_balance(EAVE, &BOB), 950);
		assert_eq!(Tokens::free_balance(EAVE, &CAROL), 1000);
		...
		assert_eq!(Tokens::free_balance(EAVE, &BOB), 1000);
		assert_eq!(Tokens::free_balance(EAVE, &CAROL), 945);
		let bob_ref_count_2 = System::consumers(&BOB);
		assert_eq!(bob_ref_count_2, bob_ref_count_1 - 1);
		let carol_ref_count_1 = System::consumers(&CAROL);
		assert_eq!(carol_ref_count_1, carol_ref_count_0 + 1);
...
fn collateral_auction_end_handler_in_reverse_stage() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 800);
...
fn collateral_auction_end_handler_by_dex_which_target_not_zero() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DEXModule::get_swap_target_amount(&[BTC, EUSD], 100, None).unwrap(), 500);

		assert_eq!(SHYTreasuryModule::total_collaterals(BTC), 100);
		assert_eq!(AuctionManagerModule::total_target_in_auction(), 200);
		assert_eq!(AuctionManagerModule::total_collateral_in_auction(BTC), 100);
		assert_eq!(Tokens::free_balance(BTC, &BOB), 1000);
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 980);
		assert_eq!(Tokens::free_balance(EUSD, &ALICE), 1000);
		...
		assert_eq!(Tokens::free_balance(BTC, &BOB), 1000);
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 1000);
		assert_eq!(Tokens::free_balance(EUSD, &ALICE), 1300);
...
fn debit_auction_end_handler_with_bid() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Tokens::free_balance(EAVE, &BOB), 1300);
		assert_eq!(Tokens::total_issuance(EAVE), 3300);
		assert_eq!(AuctionManagerModule::debit_auctions(0), None);
		assert_eq!(AuctionManagerModule::total_debit_in_auction(), 0);
...
fn surplus_auction_end_handler_with_bid() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(AuctionManagerModule::total_surplus_in_auction(), 100);
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 1000);
		assert_eq!(Tokens::free_balance(EAVE, &BOB), 500);
		assert_eq!(Tokens::total_issuance(EAVE), 2500);
		...
		assert_eq!(AuctionManagerModule::surplus_auctions(0), None);
		assert_eq!(AuctionManagerModule::total_surplus_in_auction(), 0);
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 1100);
		assert_eq!(Tokens::total_issuance(EAVE), 2500);
...
fn cancel_collateral_auction_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Tokens::free_balance(EUSD, &BOB), 920);
		assert_eq!(SHYTreasuryModule::total_collaterals(BTC), 10);
