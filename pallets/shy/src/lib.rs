...
//! ## Overview
//!
//! The entry of the SHY protocol for users, user can manipulate their SHY
...
use eave_primitives::{Amount, CurrencyId};
use sp_runtime::{
	traits::{StaticLookup, Zero},
	DispatchResult,
};
use eave_pallet_support::EmergencyShutdown;
...
pub mod module {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + eave_pallet_shy_engine::Config {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn adjust_loan(
			...
			<eave_pallet_shy_engine::Module<T>>::adjust_position(&who, currency_id, collateral_adjustment, debit_adjustment)?;
			Ok(().into())
		}
		...
		pub fn transfer_loan_from(
			...
			<eave_pallet_loans::Module<T>>::transfer_loan(&from, &to, currency_id)?;
			Ok(().into())
		}
		...
		#[pallet::weight(<T as Config>::WeightInfo::unauthorize_all(<T as eave_pallet_shy_engine::Config>::CollateralCurrencyIds::get().len() as u32))]
		#[transactional]
