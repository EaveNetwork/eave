...
fn dawn_session_keys(grandpa: GrandpaId, babe: BabeId) -> dawn_runtime::SessionKeys {
	dawn_runtime::SessionKeys { grandpa, babe }
}
	...
	dawn_runtime::GenesisConfig {
		...
		pallet_session: SessionConfig {
			keys: initial_authorities
				.iter()
				.map(|x| (x.0.clone(), x.0.clone(), dawn_session_keys(x.2.clone(), x.3.clone())))
				.collect::<Vec<_>>(),
		},
